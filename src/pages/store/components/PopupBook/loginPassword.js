...
import { CONFIRM_BOOKING, LOGIN } from './stepsBooking';


require('./styles.css')

const LoginPassword = ({ setStepPopup }) => {
    ...
    return (
        <>
            ...
            <div className='bodyPopupBook loginPopupBook'>
                <h3 className='titleInputLogin passwordLoginTitle'>Mi contraseña es:</h3>
                <p className='subtitleLoginPopupBook'>Introduce la contraseña que has creado para tu cuenta.</p>

                <input type="password" name="password" className='input passwordLogin' placeholder='Contraseña' />
                <div>
                    <button className='forgotPasswordButton'>¿Olvidaste la contraseña?</button>
                    <button className='changeEmailButton' onClick={() => { setStepPopup(LOGIN) }}>Cambiar correo electrónico</button>
                </div>
                <button className='continueLogin registerButton' onClick={() => { setStepPopup(CONFIRM_BOOKING) }}>Iniciar sesión</button>
            </div>
        </>
    );
}
